...
import {createEmptyTimeTableDetail} from './submit'
...
class TimetableDay extends React.Component {
  constructor(props){
   ...
   componentDidMount(){ 
    ...
    //    //console.log(button);
    //    var timeTableDetailId = button.data('id');             // Extract info from data-* attributes
    //    //console.log(button.data('single-edit'));
    //    this.setState({editMode: button.data('single-edit')}); 
    //    this.setState({timeTableDetailId});    
    //  }.bind(this));
 
    //  // call on modal close
    //  $('#timetableDayPopup').on('hidden.bs.modal', function (e) {            
    //    this.setState({timeTableDetailId : 0});
    ...
    console.log('modal before call edit page');
     axios.get('/api/GetTimeTableDetailByTimeTableID/' + this.props.timeTableId)
       .then(res=>{            
           if(res.data){
              console.log('exists..');
           }
 ...
 render() {
  const { handleSubmit, pristine, reset, submitting, timetableId } = this.props
